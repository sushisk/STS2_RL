...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("checkpoints/choice_policy_8token_best/best.pt"),
    )
...
def main() -> int:
    ...
    test_rows = [
        r
        for r in audit["in_scope_rows"]
        if audit["split_map"][r["trajectory_id"]] == "test"
    ][: args.count]
        ...
        print(
            {
                ...
                "top1_confidence": (
                    round(result["top1_confidence"], 4)
                    if result["top1_confidence"] is not None
                    else None
                ),
                "confidence_margin": (
                    round(result["confidence_margin"], 4)
    ...
    if latencies_ms:
        ...
        print()
        print(
            f"latency over {n} decisions: mean={sum(latencies_ms)/n:.4f}ms p50={latencies_ms[n//2]:.4f}ms max={latencies_ms[-1]:.4f}ms"
        )
    return 0
