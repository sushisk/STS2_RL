...
def build_value_targets(
    ...
    for trajectory_id, rows in by_trajectory.items():
        ...
        eligible = (
            last_row.get("data_usage_classification") in VALUE_ELIGIBLE_DATA_USAGE
        )
        win_target = None
        ...
        final_hp_target = (
            (final_hp / max_hp) if (final_hp is not None and eligible) else None
        )

        for row in rows:
            remaining = last_index - row["decision_index"]
            row_eligible = (
                eligible and win_target is not None and final_hp_target is not None
            )
            targets.append(
                {
                    ...
                    "next_decision_id": (
                        f"{trajectory_id}:{row['decision_index'] + 1}"
                        if row["decision_index"] < last_index
                        else None
                    ),
                    "is_terminal": row["decision_index"] == last_index
                    and last_row.get("termination_reason") == "terminal",
                    "is_truncated": row["decision_index"] == last_index
                    and last_row.get("termination_reason") != "terminal",
                    ...
                    "remaining_decisions_target": (
                        float(remaining) if row_eligible else None
                    ),
                }
            )
    return targets
...
def write_value_targets(
    export_root: Path, dataset_kind: str = "complete"
) -> dict[str, Path]:
...
class STS2ValueDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        return {
            ...
            "final_hp_target": torch.tensor(
                target["final_hp_target"], dtype=torch.float32
            ),
            "remaining_decisions_target": torch.tensor(
                target["remaining_decisions_target"] / REMAINING_DECISIONS_SCALE,
                dtype=torch.float32,
            ),
        }
...
def collate_value(items: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "remaining_decisions_target": torch.stack(
            [item["remaining_decisions_target"] for item in items]
        ),
    }
